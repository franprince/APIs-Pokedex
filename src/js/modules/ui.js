...
function mostrarPokemonSeleccionado(pokemonSeleccionado) {
	...
	for (let i = 0; i < pokemonSeleccionado.tipos.length; i += 1) {
		containerBadgeTipoPokemon.append($(`<img class="icon ${pokemonSeleccionado.tipos[i]}" alt="tipo ${pokemonSeleccionado.tipos[i]}" data-toggle="tooltip" data-placement="top" title="${pokemonSeleccionado.tipos[i]}" src="./src/img/pokemon-types/${pokemonSeleccionado.tipos[i]}.svg"/>`));
	}
